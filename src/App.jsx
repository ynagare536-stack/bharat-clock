    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '12px 16px', boxSizing: 'border-box' }}>
      ...
      <header className="cyber-panel border-gradient-purple" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 20px', marginBottom: '12px' }}>
        ...
        <nav style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          <button style={{ padding: '6px 14px', borderRadius: '20px', background: 'rgba(59, 130, 246, 0.2)', border: '1px solid #3b82f6', color: '#60a5fa', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}>⌂ Home</button>
          <button onClick={() => alert('World Clock')} style={{ padding: '6px 12px', borderRadius: '20px', background: 'transparent', border: 'none', color: '#94a3b8', fontSize: '11px', cursor: 'pointer' }}>🌐 World Clock</button>
          <button onClick={() => alert('Calendar')} style={{ padding: '6px 12px', borderRadius: '20px', background: 'transparent', border: 'none', color: '#94a3b8', fontSize: '11px', cursor: 'pointer' }}>📅 Calendar</button>
          <button onClick={() => setActiveModal('notes')} style={{ padding: '6px 12px', borderRadius: '20px', background: 'transparent', border: 'none', color: '#94a3b8', fontSize: '11px', cursor: 'pointer' }}>🛠️ Tools</button>
          <button onClick={() => setActiveModal('alarm')} style={{ padding: '6px 12px', borderRadius: '20px', background: 'transparent', border: 'none', color: '#94a3b8', fontSize: '11px', cursor: 'pointer' }}>🔔 Alarm</button>
        </nav>

        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button onClick={() => setIs24Hour(!is24Hour)} style={{ padding: '4px 10px', borderRadius: '6px', background: '#312e81', border: '1px solid #818cf8', color: '#e0e7ff', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}>
            {is24Hour ? '24H' : '12H'}
          </button>
          <span style={{ fontSize: '11px', color: '#cbd5e1', background: 'rgba(255,255,255,0.06)', padding: '5px 8px', borderRadius: '6px' }}>EN ▾</span>
        </div>
      </header>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1.3fr 1.1fr', gap: '12px' }}>
          ...
          <div className="cyber-panel border-gradient-cyan" style={{ padding: '14px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#00f0ff' }}>
              ...
              <span style={{ color: '#94a3b8', fontSize: '9px' }}>📍 New Delhi</span>
            </div>
          ...
          <div className="cyber-panel border-gradient-cyan" style={{ padding: '14px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              ...
              <button onClick={() => { setSwRunning(false); setSwTime(0); }} style={{ padding: '6px 12px', background: '#334155', border: '1px solid #64748b', color: '#fff', borderRadius: '6px', fontSize: '10px', cursor: 'pointer' }}>
                Reset
              </button>
            </div>
          </div>
          ...
          {/* TIMER WIDGET */}
          <div className="cyber-panel border-gradient-purple" style={{ padding: '14px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span className="font-tech" style={{ fontSize: '11px', color: '#fbbf24', fontWeight: 'bold' }}>⏳ TIMER</span>
              <div style={{ display: 'flex', gap: '4px' }}>
                <button onClick={() => { setTimerRunning(false); setTimerSeconds(15*60); }} style={{ padding: '2px 5px', fontSize: '8px', back
